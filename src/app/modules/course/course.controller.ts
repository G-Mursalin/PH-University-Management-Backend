...
const getAllCourses = catchAsync(async (req, res) => {
    const { result, meta } = await courseServices.getAllCourses(req.query);
...
//Course Faculties Model

const getFacultiesCourse = catchAsync(async (req, res) => {
    const { courseId } = req.params;
    const result = await courseServices.getFacultiesCourse(courseId);

    sendResponse(res, {
        statusCode: httpStatus.OK,
        success: true,
        message: 'Course with faculties retrieved successfully',
        data: result,
    });
});
